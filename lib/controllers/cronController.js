    ...
    const job = new cronJob('0 0 5 * * *', async function () {
      ...
      if (allMemberSub.length !== 0) {
        for (let i of allMemberSub) {
          ...
          const id = i.id;
          const tkb = await _humgAPI.default.getSchedule(msv, name, id);

          if (tkb.length > 72) {
            await _facebookAPI.default.callSendAPI(id, "H\xF4m nay ".concat(_humgAPI.default.getFullDate(), ", ").concat(name, " ph\u1EA3i h\u1ECDc:"));
            await _facebookAPI.default.callSendAPI(id, tkb);
            await _facebookAPI.default.callSendAPI(id, "Nh\u1EDB \u0111i h\u1ECDc \u0111\u1EA7y \u0111\u1EE7 v\xE0 \u0111\xFAng gi\u1EDD nha... Y\xEAu ".concat(name, " 3000 \u2764"));
          } else {
            await _facebookAPI.default.callSendAPI(id, tkb);
          }
        }
